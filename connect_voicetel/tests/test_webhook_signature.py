...
webhook validation accepts — even though connect_voicetel never imports the
...
class TestWebhookSignature(unittest.TestCase):
    ...
    def test_roundtrip_valid(self):
        url = 'https://example.com/voicetel/webhook/callstatus'
    ...
    def test_port_tolerant(self):
        # Signing has historically been inconsistent about the port, so a
        # signature computed on the no-port URL validates against the
        # with-port URL.
        no_port = 'https://example.com/voicetel/webhook/callstatus'
        sig = webhook.sign_request(no_port, {'Digits': '1234'}, '12345')
        self.assertTrue(webhook.valid_request(
            'https://example.com:8443/voicetel/webhook/callstatus',
