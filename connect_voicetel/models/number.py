...
class VoicetelNumber(models.Model):
    _name = 'connect.voicetel.number'
    _description = 'VoiceTel Phone Number'
    ...
    callflow = fields.Many2one('connect.voicetel.callflow', ondelete='set null')
    user = fields.Many2one('connect.user', ondelete='set null')
    application = fields.Many2one('connect.voicetel.application', ondelete='set null')
    ...
    def _get_voicetel_urls(self):
        ...
        for rec in self:
            ...
            rec.voice_url = urljoin(api_url, 'voicetel/webhook/number')
            rec.voice_fallback_url = (
                urljoin(fallback_url, 'voicetel/webhook/number') if fallback_url else ''
            )
    ...
    def write(self, vals):
        ...
        for rec in self:
            if not self.env.context.get('skip_voicetel_sync'):
    ...
    def render(self, request={}, params={}):
        ...
        if self.destination == 'application' and self.application:
            ...
            return self.user.render_voicetel(request)
    ...
    def route_call(self, request, params={}):
        ...
        self.env['connect.call'].on_call_status_voicetel(request)
