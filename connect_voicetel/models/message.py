...
class VoicetelMessage(models.Model):
    ...
    # super() call either way. Only our own /voicetel/webhook/message*
    # controllers call this name, so it can never be shadowed.
    @api.model
    def receive_voicetel(self, params):
        try:
    ...
    def send(self, recipient, body, res_id=None, res_model=None,
        ...
        if self.env['connect.settings']._get_message_provider() != 'voicetel':
            return super().send(
        ...
        if outgoing_callerid:
            ...
            number = sender_user.connect_user.voicetel_outgoing_callerid
            if not number:
