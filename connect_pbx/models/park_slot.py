...
class ParkSlot(models.Model):
    ...
    def init(self):
        ...
        if self.env.cr.fetchone()[0]:
            # sync_slots reads connect.settings, whose audio columns this
            # module adds. The savepoint keeps a failed read from aborting the
            # surrounding install transaction.
            try:
                with self.env.cr.savepoint():
                    self.sync_slots()
            except Exception as e:
                logger.warning('park_slot init: could not sync slots: %s', e)
