...
class TestVoicemailBoxModel(ConnectTestCase):
    ...
    def test_scheduled_calls_require_connect_administration(self):
        """Scheduled-call records are restricted to telephony administrators."""
        Scheduled = self.env['connect.scheduled_call']
        with self.assertRaises(AccessError):
            Scheduled.with_user(self.alice).create({'calling_user': self.alice.id})
        self.alice.group_ids = [(4, self.env.ref('connect.group_admin').id)]
        call = Scheduled.with_user(self.alice).create({'calling_user': self.alice.id})
        self.assertEqual(call.calling_user, self.alice)
    ...
    def test_voicemail_download_uses_provider_media_auth_without_redirects(self):
        ...
        settings.set_param('account_sid', account_sid)
