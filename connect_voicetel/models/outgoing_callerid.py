...
class VoicetelOutgoingCallerId(models.Model):
    _name = 'connect.voicetel.outgoing_callerid'
    _description = 'VoiceTel Outgoing Caller ID'
    _rec_name = 'number'
    _order = 'number'
    ...
    def _raw_client(self):
        ...
        ValidationRequests, so this talks to the VoiceTel API directly with
        the same HTTP Basic (account SID : API key) auth the SDK uses
        everywhere else — an SDK gap, not a design choice; delete this in
        favour of the SDK once it wraps the resource.
        """
        settings = self.env['connect.settings'].sudo()
        base = settings._voicetel_base_url()
        account_sid = settings.get_param('voicetel_account_sid')
        api_key = settings.get_param('voicetel_api_key')
        if not (base and account_sid and api_key):
            raise ValidationError('Set Account SID and API key first!')
        return httpx.Client(
    ...
    def sync(self):
        ...
        # Numbers already synced by connect.voicetel.number are also usable
        # as caller IDs; mirror them here so the dropdown is complete.
    ...
    def update_status(self, params):
        ...
        if not number:
            return False
        rec = self.search(
            [('number', '=', number), ('callerid_type', '=', 'outgoing_callerid')], limit=1)
        if rec:
