...
records to connect_pbx before connect's own data load runs.
...
table with it. Re-owning them here, before the load, makes connect_pbx
...
def migrate(cr, version):
    ...
    logger.info('connect_pbx: re-owned %s ir.model rows', cr.rowcount)
    ...
    logger.info('connect_pbx: re-owned %s ir.model.fields rows', cr.rowcount)
    ...
    logger.info('connect_pbx: re-owned %s selection rows', cr.rowcount)
    ...
    logger.info('connect_pbx: re-owned %s xml records', cr.rowcount)
    ...
    cr.execute(
        ...
        UPDATE ir_config_parameter
           SET value = 'connect_pbx.audio_qa_recording_notice'
         WHERE key = 'connect.qa_recording_notice_audio'
           AND value = 'connect.audio_qa_recording_notice'
        """
    )
    logger.info('connect_pbx: repointed %s qa-notice parameters', cr.rowcount)

    # 5. Constraints and m2m relation tables follow their model, so the
    #    uninstall bookkeeping drops them with connect_pbx rather than
    #    with connect.
    ...
    if row:
        ...
        logger.info('connect_pbx: re-owned %s constraints', cr.rowcount)
        cr.execute(
            "UPDATE ir_model_relation SET module = %s WHERE name = %s",
            (module_id, 'connect_voicemail_box_member_rel'),
        )
        logger.info('connect_pbx: re-owned %s m2m relations', cr.rowcount)
    else:
        # connect_pbx has no ir_module_module row yet on the very first
        # upgrade that introduces it; the reflection pass on install claims
        # the constraints and relations itself.
        logger.info(
            'connect_pbx not yet registered; constraint and relation '
