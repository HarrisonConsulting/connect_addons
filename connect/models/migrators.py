...
class DomainMigrator(Migrator):
    ...
    def run(self, dest_client, dry_run):
        ...
        for rec in records:
            ...
            try:
                # The savepoint is what makes the per-item isolation below
                # real. Without it a database-level error (e.g. a username
                # unique violation while reconciling credentials) aborts the
                # transaction, and every later item dies with
                # InFailedSqlTransaction no matter what this except does.
                    ...
                    if dest and dest.sid == rec.sid:
                        ...
                        if not dry_run:
                            try:
                                ...
                                # Match on the 409, not on a substring: the
                                # provider's real wording is "The SIP
                                # DomainName '…' is already registered".
                                if rec._is_twilio_domain_conflict(e):
                                    rec._import_existing_domain_by_name(dest_client)
                                else:
                                    raise
