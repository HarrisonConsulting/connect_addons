...
class VoicetelApplication(models.Model):
    _name = 'connect.voicetel.application'
    _description = 'VoiceTel Application'
    _order = 'name'
    ...
    code_type = fields.Selection([
        ('twiml', 'Call Control XML'),
        ('model_method', 'model.method'),
    ], required=True, default='twiml')
    twiml = fields.Text(
        required=True, string='Call Control XML',
        default='<?xml version="1.0" encoding="UTF-8"?><Response><Say>Hello</Say></Response>',
        help="VoiceTel's call-control XML (Twilio-compatible).")
    model = fields.Char()
    method = fields.Char()
    voice_url = fields.Char(compute='_get_voicetel_urls', compute_sudo=True)
    voice_fallback_url = fields.Char(compute='_get_voicetel_urls', compute_sudo=True)
    voice_status_url = fields.Char(compute='_get_voicetel_urls', compute_sudo=True)
    exten = fields.Many2one('connect.voicetel.exten', ondelete='set null', readonly=True)

    def _get_voicetel_urls(self):
        settings = self.env['connect.settings']
        api_url = settings.get_param('api_url')
        fallback_url = settings.get_param('api_fallback_url')
        for rec in self:
            rec.voice_url = urljoin(api_url, 'voicetel/webhook/application/{}'.format(rec.id))
            rec.voice_status_url = urljoin(api_url, 'voicetel/webhook/callstatus')
            rec.voice_fallback_url = (
                urljoin(fallback_url, 'voicetel/webhook/application/{}'.format(rec.id))
    ...
    def create_application(self, client):
        ...
        debug(self, 'Created VoiceTel application {}.'.format(self.name))
        return application
    ...
    def write(self, vals):
        ...
        if not self.env['connect.settings'].get_param('voicetel_auto_sync'):
            return res
        client = self.env['connect.settings']._voicetel_client()
        for rec in self:
            if rec.sid:
        ...
        for rec in self:
            if rec.sid:
                try:
                    ...
                    logger.exception('Failed to delete VoiceTel application %s', rec.sid)
        return super().unlink()
