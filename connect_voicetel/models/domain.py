...
class VoicetelDomain(models.Model):
    _name = 'connect.voicetel.domain'
    _description = 'VoiceTel SIP Domain'
    ...
    def _get_voicetel_urls(self):
        ...
        for rec in self:
            rec.voice_url = urljoin(api_url, 'voicetel/webhook/domain')
    ...
    def create_domain(self, client):
        ...
        debug(self, 'Created VoiceTel SIP domain {}.'.format(self.domain_name))
        return self.sid
    ...
    def route_call(self, request, params={}):
        ...
        # SIP username arrives in To: sip:<user>@<domain>.
        to = request.get('To', '')
        uri_user = to.split('@')[0].split(':')[-1] if '@' in to else ''
        if uri_user:
            user = self.env['connect.user'].sudo().search(
                [('voicetel_username', '=', uri_user)], limit=1
            )
            if user:
                return user.render_voicetel(request, params)
